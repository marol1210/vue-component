...
export default {
  ...
  mounted() {
    ...
    let $this = this
    this.axiosRequest({url:"/graphql",method:"post",data:{"query":"{allCategory{id name slug created_at meta_title}}"}}).then(function(response){
																														        	if(response.data.errors){
																														        		console.log(response.data.errors[0].message)
																														        	}else{
																														        		$this.$set($this.categories,'data',response.data.data.allCategory)
																														        	}
																														        });
  },
  methods: {
