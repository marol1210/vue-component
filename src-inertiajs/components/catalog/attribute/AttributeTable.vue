...
export default {
    ...
    mounted() {
        ...
        let $this = this
        this.axiosRequest({url:"/graphql",method:"post",data:{"query":"{allAttribute{id name slug display_as}}"}})
	        .then(function(response){
	        	if(response.data.errors){
	        		console.log(response.data.errors[0].message)
	        	}else{
	        		$this.$set($this.attributes,'data',response.data.data.allAttribute)
	        	}
	        });
    },
    methods: {
