    ...
    <div>
        ...
        >
            <template slot="name" slot-scope="{ item }">
                <a
                    :href="`${share_param.url.base_url}/product/${item.id}/edit`"
...
export default {
    ...
    mounted() {
        ...
        let $this = this
        this.axiosRequest({url:"/graphql",method:"post",data:{"query":"{allProduct{id name slug created_at price}}"}})
	        .then(function(response){
	        	if(response.data.errors){
	        		console.log(response.data.errors[0].message)
	        	}else{
	        		$this.$set($this.products,'data',response.data.data.allProduct)
	        	}
	        });
    }
};
